...
_LICENSE_DIR  = os.path.join(str(Path.home()), ".SecretPulse")
...
def _key_validate(key: str) -> Tuple[bool, str, Optional[dict]]:
    ...
    if now > expires:
        ...
        return False, f"License expired on {exp_date}. Renew at https://SecretPulse.sh", None
...
def _key_load() -> Optional[str]:
    ...
    env_key = os.environ.get("SecretPulse_KEY")
    if env_key:
...
KNOWN_TEST_KEYS = {
    ...
    'sk_live_EXAMPLE_KEY_DO_NOT_USE_1',
    'pk_live_EXAMPLE_KEY_DO_NOT_USE_2',
    'sk_test_EXAMPLE_KEY_DO_NOT_USE_3',
    'pk_test_EXAMPLE_KEY_DO_NOT_USE_4',
    'YOUR_WRITE_KEY',
...
class AccuracyEngine:
    ...
    def __init__(self):
        ...
        self.calibration_file = os.path.join(str(Path.home()), ".SecretPulse", "calibration.json")
        ...
        self.known_secrets_db = os.path.join(str(Path.home()), ".SecretPulse", "known_secrets.json")

        # ── Upgrade 4: Cross-Repo Leak Correlation ──
        self.cross_repo_db = os.path.join(str(Path.home()), ".SecretPulse", "cross_repo.json")
    ...
    def calculate_confidence_score(finding: dict, context_result: dict = None,
        ...
        """
        SecretPulse Secret Confidence Formula v2
        ═════════════════════════════════════
    ...
    def calibration_analyze(calibration_file: str) -> dict:
        ...
        Run: python3 -c "from SecretPulse import AccuracyEngine; AccuracyEngine.calibration_report()"
...
class TeamNotifier:
    ...
    def _send_generic(self, findings, stats, target, critical, high, live):
        payload = {
            'tool': f'SecretPulse v{__version__}',
...
class SecretPulseDB:
    ...
    DB_DIR = Path.home() / '.SecretPulse'
    DB_PATH = DB_DIR / 'SecretPulse.db'
...
class SmartRouter:
    ...
    Most tools scan everything everywhere. SecretPulse chooses.
    """
...
class ReconOrchestrator:
    ...
    def __init__(self, session, db: SecretPulseDB = None, verbose: bool = False,
            ...
            try:
                ...
                if resp and resp.status_code == 200:
                    ...
                    if '<ListBucketResult' in body or '<Contents>' in body:
                        ...
                        test_key = f"SecretPulse-security-test-{int(time.time())}.txt"
                        try:
                            self.rate_limiter.wait()
                            put_resp = self.session.put(
                                f"{base_url}/{test_key}",
                                data="SecretPulse security test - safe to delete",
                                timeout=8
                            )
        ...
        'runs': [{
            'tool': {
                'driver': {
                    'name': 'SecretPulse',
                    'version': __version__,
                    'informationUri': 'https://github.com/muizzamri/SecretPulse',
...
<h1>🔍 SecretPulse v{__version__} — Report</h1>
...
<div class="footer">SecretPulse v{__version__} by {__author__} — For Authorized Bug Bounty Testing Only</div>
</body></html>"""
...
def print_banner():
    ...
    # SecretPulse in hex = 53 45 43 52 45 54 20 50 55 4C 53 45
    hex_sig = f"{ch}53 45 43 52 45 54 20 50 55 4C 53 45{R}"

    # Logo lines (ASCII art for SECRET PULSE)
    L = [
        f"{c1}{B}  ██████  ███████  ██████  ██████  ███████  ████████ {R}",
        f"{c2}{B} ██       ██      ██      ██   ██ ██          ██    {R}",
        f"{c3}{B}  █████   █████   ██      ██████  █████       ██    {R}",
...
def main():
    parser = argparse.ArgumentParser(
        description=f'SecretPulse v{__version__} — High-Signal Secret & API Exposure Scanner',
  ...
  %(prog)s --generate-github-action
    ...
    if args.db_stats:
        try:
            db = SecretPulseDB()
            stats = db.get_stats()
            print(f"\n  SecretPulse Database Statistics")
            print(f"  {'=' * 35}")
    ...
    db = SecretPulseDB() if args.db else None
    ...
    if args.db:
        print(f"[*] SQLite persistence: ON (~/.SecretPulse/SecretPulse.db)")
    if args.incremental:
        ...
        if target_domain:
            ...
            if discovered:
                ...
                if args.recon_only:
                    ...
                    if db:
                        print(f"    Results saved to ~/.SecretPulse/SecretPulse.db")
    ...
    if scanner.found_secrets:
        ...
        if cal_count:
            print(f"{Colors.DIM}    Calibration: {cal_count} findings logged to ~/.SecretPulse/calibration.json{Colors.ENDC}")
